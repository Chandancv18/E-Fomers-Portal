...
app.get('/home', function (req, res) {  
   ...
   res.send('<h1>WELCOME TO E-FORMERS PORTAL </h1>');   
})

app.get('/about', function (req, res) {  
   console.log("Got a GET request for /about");  
    res.send(' <h1>  YOU CAN CHECK E-FORMERS PORTAL </h1>');})  
...
app.post('/process_post', function (req, res) {
    ...
    req.body.serverMessage = "NodeJS replying to angular"
		...
		console.log("Sent data are (POST): Former ID :"+req.body.pasid+" Former mobile number="+req.body.pasname +"Former current Place  " +req.body.pasg +" Former Loan?"+req.body.pasno+" Crops Growing "+req.body.src+"Manure Using"+req.body.dst);
		// Submit to the DB
...
/*Sending the respone back to the angular Client */
                        res.end("Former  Inserted-->"+JSON.stringify(newpas));
					}
                })      
}) 
        ...
        data.update({"pasname":pasname1},{$set:{"pasname":"newpas"}},{multi:true},
            function(err, obj){
				...
				res.send("<br/>"+pasname1+":"+"<b>Passenegr Name Updated</b>");
                    console.log("Former Updated")
				}
				else
                    res.send("FOrmer Not Found")
			}
        });
    });
})	
...
console.log("");
console.log("Please see the command Prompt After inserting pFormers Details And After any operations ");
